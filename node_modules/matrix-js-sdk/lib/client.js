...
class MatrixClient extends _typedEventEmitter.TypedEventEmitter {
  ...
  getEventTimeline(timelineSet, eventId) {
    ...
    const promise = this.http.authedRequest(undefined, _httpApi.Method.Get, path, params).then(async res => {
      // TODO types
      if (!res.event) {
      ...
      await this.processThreadEvents(timelineSet.room, threadedEvents, true); // there is no guarantee that the event ended up in "timeline" (we
  ...
  paginateEventTimeline(eventTimeline, opts) {
    ...
    if (isNotifTimeline) {
      ...
      undefined, _httpApi.Method.Get, path, params, undefined).then(async res => {
        ...
        await this.processThreadEvents(timelineSet.room, threadedEvents, backwards); // if we've hit the end of the timeline, we need to stop trying to
  ...
  processRoomEventsSearch(searchResults, response) {
    ...
    searchResults.highlights = Array.from(highlights); // append the new results to our existing results
  ...
  * @return {Object} the response, with chunk, prev_batch and, next_batch.
  ...
  setGroupProfile(groupId, profile) {
    ...
    return this.http.authedRequest(undefined, _httpApi.Method.Post, path, undefined, profile);
  }
  /**
   * @param {string} groupId
  ...
  setGroupJoinPolicy(groupId, policy) {
    ...
    return this.http.authedRequest(undefined, _httpApi.Method.Put, path, undefined, {
  ...
  removeUserFromGroupSummary(groupId, userId) {
    ...
    return this.http.authedRequest(undefined, _httpApi.Method.Delete, path, undefined, {});
  }
  /**
   * @param {string} groupId
   * @param {string} roomId
  ...
  acceptGroupInvite(groupId, opts = null) {
    ...
    return this.http.authedRequest(undefined, _httpApi.Method.Put, path, undefined, opts || {});
  }
  /**
   * @param {string} groupId
   * @return {Promise} Resolves: Empty object
  ...
  eventShouldLiveIn(event, room, events, roots) {
    ...
    const parentEvent = (room === null || room === void 0 ? void 0 : room.findEventById(parentEventId)) || events.find(mxEv => mxEv.getId() === parentEventId); // A reaction targetting the thread root needs to be routed to both the
    ...
    if (parentEvent !== null && parentEvent !== void 0 && parentEvent.getAssociatedId()) {
